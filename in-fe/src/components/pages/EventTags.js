    ...
    <TuiForm style={{ margin: "20px" }}>
      <TuiFormGroup>
        ...
        <TuiFormGroupContent>
          ...
          <TuiFormGroupField
            header="Tags"
            description="These are the tags for your selected event type."
          >
            <TuiTagger
              label="Tags"
              tags={tagsObj.tags}
  ...
  const handleSave = async () => {
    try {
      ...
      if (objToSend.type) {
        ...
        setErrorMessage("Event type cannot be empty.");
    } catch (e) {
      if (mounted.current) setError(e);
    } finally {
      if (mounted.current) setLoading(false);
    ...
    <TuiForm style={{ margin: "20px" }}>
      <TuiFormGroup>
        ...
        <TuiFormGroupContent>
          ...
          <TuiFormGroupField
            header="Event type"
            description="Select event type to be tagged."
          >
            <TuiSelectEventType
              onSetValue={(value) =>
                setObjToSend({ ...objToSend, type: value })
              }
    ...
    <div>
      ...
      {tags.length === 0 && (
        <NoData header="There is no data here.">
          <p>Please click New Tags button in the upper right corner.</p>
        </NoData>
      )}
      {tags.length !== 0 && (
        <TuiForm>

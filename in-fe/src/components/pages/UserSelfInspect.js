  ...
  return (
    <TuiForm style={{ margin: 20 }}>
      <TuiFormGroup>
        <TuiFormGroupHeader
          header="Edit account"
          description="Here you can edit some of your account data. If you want to edit data that is not present below, please contact ReBid Insights admin."
        />
        {errors && <ErrorsBox errorList={errors} />}
        <TuiFormGroupContent>
          <TuiFormGroupField
            header="Full name"
            description="Here you can edit your full name."
          >
            <TextField
              fullWidth
              variant="outlined"
              ...
              onChange={(event) =>
                setUserToSend({ ...userToSend, full_name: event.target.value })
              }
              size="small"
            />
          </TuiFormGroupField>
          <TuiFormGroupField
            header="Password"
            description="Here you can edit your password."
          >
            <PasswordInput
              fullWidth
              variant="outlined"
              ...
              onChange={(event) =>
                setUserToSend({ ...userToSend, password: event.target.value })
              }
              size="small"
              error={error && userToSend.password !== confirmPassword}
              helperText={
                error &&
                userToSend.password !== confirmPassword && (
                  <p className="helper_error">Given passwords do not match</p>
                )
              }
            />
          </TuiFormGroupField>
          <TuiFormGroupField
            ...
            description="Please type in the password one more time."
          >
            <PasswordInput
              fullWidth
              variant="outlined"
...
export default function UserSelfInspect() {
  ...
  return (
    <>
      <TuiForm className="m-5">
        <TuiFormGroup>
          <TuiFormGroupHeader
            header={`Hello ${user.full_name}`}
            description="Here you can see and edit your account."
          />
          <TuiFormGroupField className="m-5">
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              >
                <div>
                  ...
                  <Box
                    sx={{
                      backgroundColor: user?.disabled ? "#d81b60" : "#00c853",
                      height: 10,
                      width: 100,
                    }}
                  />
      ...
      <FormDrawer open={edit} width={600} onClose={() => setEdit(false)}>
        {edit && (
          <UserSelfEditForm
            user={user}

...
function NodeDetailsHandler({
  ...
  return (
    <NoData header="Available only as Insights Pro service">
      <p style={{ textAlign: "center" }}>
        Please join Insights Pro for free and premium connectors and services.
...
export function FlowEditorPane({
  ...
  const [displayDebugHeight, setDisplayDebugHeight] = useState({
    gridTemplateRows: "calc(100% - 33px) 33px",
  });
  ...
  const updateFlow = useCallback(
    (data) => {
      if (data) {
        ...
        if (showAlert) {
          showAlert({
            message: "This workflow is missing",
            type: "warning",
            hideAfter: 2000,
          });
        } else {
  ...
  const handleDraftSave = useCallback(
    (deploy = false) => {
      if (reactFlowInstance) {
        ...
        showAlert({
          message: "Can not save Editor not ready.",
          type: "warning",
          hideAfter: 2000,
        });
      }
  ...
  const onNodeClick = (event, element) => {
    ...
    if (
      ...
      Array.isArray(element.data?.debugging) &&
      element.data?.debugging.length > 0 &&
      element.data?.debugging[0]?.edge?.id
    ) {
            ...
            <div className="FlowPane" ref={reactFlowWrapper}>
              ...
              {!flowLoading && elements && (
                ...
                >
                  ...
                  {displayNodeContextMenu && currentNode?.data?.spec?.form && (
                    <div
                      className="NodeContextForm"
                      style={{
                        left: clientX,
                        top: clientY,
                      }}
...
export default connect(mapProps, { showAlert })(FlowEditorPane);

...
export default function RuleForm({ onEnd, init }) {
  ...
  const [type, setType] = useState(
    init?.event?.type ? { name: init.event.type, id: init.event.type } : {}
  );
  ...
  const handleSubmit = async () => {
    if (
      isEmptyObjectOrNull(type) ||
      isEmptyObjectOrNull(source) ||
      isEmptyObjectOrNull(flow) ||
      !name
    ) {
  ...
  return (
    <TuiForm style={{ margin: 20 }}>
      <TuiFormGroup>
        <TuiFormGroupHeader
          header="Routing rule settings"
          description="Workflow engine will trigger selected flow
        ...
        <TuiFormGroupContent>
          <TuiFormGroupField
            header="Event type"
            description="Type event type to filter incoming events. If there
                are no events please start collecting data first."
          >
            <TuiSelectEventType
              value={type}
              errorMessage={typeErrorMessage}
              onSetValue={handleType}
              solo={false}
            />
          </TuiFormGroupField>
          <TuiFormGroupField
            header="Source"
            description="Select event source. Event without selected source will be
                    discarded."
          >
      ...
      <TuiFormGroup>
        ...
        <TuiFormGroupContent>
          <TuiFormGroupField
            header="Name"
            description="Rule name can be any string that
                    identifies rule."
          >
            <TextField
              label={"Rule name"}

  ...
  const ProfileRawData = ({ profileId }) => {
    ...
    return (
      <>
        ...
        {errors === null && profileData && (
          <ObjectInspector data={profileData} theme={theme} expandLevel={3} />
        )}
      </>
    );
  };
      ...
      <div className="RightTabScroller">
        ...
        >
          <TabCase id={0}>
            <TuiForm style={{ padding: 20 }}>
              <TuiFormGroup>
                ...
                <TuiFormGroupContent>
                  {!isEmptyObject(data.event?.metadata) && (
                    <TuiFormGroupField header="Metadata">
                      <Properties properties={data.event?.metadata} />
                    </TuiFormGroupField>
                  )}
                  {!isEmptyObject(data.event?.properties) && (
                    <TuiFormGroupField header="Properties">
              ...
              <TuiFormGroup>
                <TuiFormGroupHeader
                  header="Event source"
                  description="Displays the resource the event data was collected from."
                />
                <TuiFormGroupContent>
                  {!isEmptyObject(data.event?.source) && (
          ...
          <TabCase id={1}>
            <TuiForm style={{ padding: 20 }}>
              <TuiFormGroup>
                ...
                <TuiFormGroupContent>
                  {!isEmptyObjectOrNull(data?.event?.profile) && (
                    <TuiFormGroupField header="Profile">
                      <Properties properties={data?.event?.profile} />
                    </TuiFormGroupField>
                  )}
                  {!isEmptyObjectOrNull(
                    data?.event?.session?.context?.page
                  ) && (
                    <TuiFormGroupField header="Page">
                      <Properties
                        properties={data?.event?.session?.context?.page}
                      />
                    </TuiFormGroupField>
                  )}
                  {!isEmptyObjectOrNull(
                    data?.event?.session?.context?.browser
                  ) && (
                    <TuiFormGroupField header="Browser">
                      <Properties
                        properties={data?.event?.session?.context?.browser}
                      />
                    </TuiFormGroupField>
                  )}
                  {!isEmptyObjectOrNull(
                    data?.event?.session?.context?.screen
                  ) && (
                    <TuiFormGroupField header="Screen">
                      <Properties
                        properties={data?.event?.session?.context?.screen}
                      />
                    </TuiFormGroupField>
                  )}
                  {!isEmptyObjectOrNull(
                    data?.event?.session?.context?.storage
                  ) && (
                    <TuiFormGroupField header="Storage">
                      <Properties
                        properties={data?.event?.session?.context?.storage}
                      />
                    </TuiFormGroupField>
                  )}
                </TuiFormGroupContent>
              </TuiFormGroup>
              {!isEmptyObjectOrNull(data?.event?.aux) && (
                <TuiFormGroup>
          ...
          <TabCase id={5}>
            <div style={{ padding: 20, height: "inherit" }}>
              <ProfileLogDetails
                profileId={data?.event?.profile?.id}
          ...
          <TabCase id={6}>
            <TuiForm style={{ padding: 20 }}>
              <TuiFormGroup>
                <TuiFormGroupHeader
                  header="Profile raw data"
                  description="This is current profile state. Not the state when the event was collected."
                />
                <TuiFormGroupContent>

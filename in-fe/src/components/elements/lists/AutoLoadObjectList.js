...
const AutoLoadObjectList = ({
  ...
  const [enableTrigger, setTrigger] = useState(false);
  ...
  const loadData = useCallback(
    (fresh = false, progress = false) => {
      ...
      if (onLoadRequest?.data?.where !== lastQuery) {
        ...
        endpoint = {
          ...onLoadRequest,
          url: `${onLoadRequest.url}/page/${page}`,
        };
      }
  ...
  function testClick() {
    ...
    window.response.context.debugger = true;
    window.response.context.saveEvent = true;
    window.response.context.saveSession = true;
    window.tracker.track(
      "rebid-insights-test-button",
      { age: 14 },
      { fire: true }
    );
  }
  ...
  return (
    <div className="ObjectList">
      {/* <button onClick={() => setTrigger(true)}>Trigger test</button> */}
      {renderHeader(timeFieldLabel)}
      {progress && <div style={{ height: 5 }}><LinearProgress /></div>}
      <InfiniteScroll
        ...
        className="h-[calc(100vh-490px)] overflow-auto"
        loader={
          <div
            style={{
              display: "flex",
      ...
      >
        <div className="Event_container">{renderRows(rows)}</div>
      </InfiniteScroll>
    </div>
  );
};
